...
interface HeaderProps {
  ...
  user: any;
  setIsProfileOpen: React.Dispatch<React.SetStateAction<boolean>>; // Fixed: Added to match page.tsx passing down the state modifier
  t: any;
}

export function Header({ lang, setLang, user, setIsProfileOpen, t }: HeaderProps) {
  // Safe extraction of parameters since the parent hook provides them down into the context stream
    ...
    <header className="shrink-0 bg-white border-b px-4 sm:px-6 py-3 z-[100] shadow-sm pt-[calc(0.75rem+env(safe-area-inset-top))]">
      <div className="max-w-5xl mx-auto flex items-center justify-between relative">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-1">
              <span className="text-[11px] sm:text-xs text-indigo-600 font-black uppercase tracking-[0.1em]">
                {t?.headerLabel || 'Credit Card Expert'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4 relative">
          ...
          {/* Profile Controls Action Trigger */}
          <div className="relative flex items-center">
            <button 
              onClick={() => setIsProfileOpen(true)} // Fixed: Triggers the sidebar drawer control safely
              className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs sm:text-sm font-black transition-all shadow-md border ${
            ...
            >
              ...
              <span className="hidden xs:inline">
                {user ? (userProfile?.fullName || t?.profile || 'Profile') : (t?.login || 'Login')}
              </span>
            </button>

            {/* Account Nudge Tier Tooltip Banner */}
            {userTier !== 'VIP' && (
              <div className="absolute top-full right-0 mt-3 md:mt-0 md:top-1/2 md:-translate-y-1/2 md:right-auto md:left-full md:ml-4 w-44 md:w-52 bg-indigo-600/80 backdrop-blur-md text-white text-[11px] sm:text-xs font-bold p-3 rounded-xl shadow-xl z-[200]">
