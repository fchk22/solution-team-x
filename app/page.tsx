    ...
    <div className="h-dvh bg-slate-50 text-slate-900 flex flex-col relative font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto w-full max-w-4xl mx-auto p-4 sm:p-6 [&::-webkit-scrollbar]:hidden">
          ...
          <div className="min-h-full flex flex-col justify-center items-center text-center py-10 animate-in fade-in zoom-in duration-500">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-xl">
                {t.quickTips.map((q: string) => (
                  <button key={q} onClick={() => sendMessage(q)} className="bg-white border-2 border-slate-100 p-4 sm:p-5 rounded-2xl text-xs sm:text-sm font-bold text-slate-700 hover:border-indigo-400 active:bg-indigo-50 transition-all text-left flex justify-between items-center group">
                    {q} <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-indigo-500 transition-transform" />
                  </button>
                ))}
             </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          {loading && <div className="text-[10px] sm:text-xs text-slate-400 font-bold animate-pulse flex items-center gap-2 px-2"><Bot className="w-4 h-4"/> {t.thinking}</div>}
