...
export function ChatInterface({ 
  ...
  // 🎯 FORCEFUL EXTRACTOR: Looks strictly at what is written in the message body text 
  // rather than trusting state parameters that might be stuck on old/default values.
  const extractCardFromContent = (content: string): string => {
    const text = content.toLowerCase();
    
    if (text.includes('hang seng') || text.includes('恒生') || text.includes('恆生')) {
      return 'hang_seng_enj_mastercard';
    }
    if (text.includes('icbc') || text.includes('工銀') || text.includes('工银')) {
      return 'icbc_constellation_visa';
    }
    if (text.includes('standard chartered') || text.includes('渣打') || text.includes('cathay')) {
      return 'standard_chartered_cathay_mastercard';
    }
                      ...
                      if (isApplyLink) {
                        return (
                          <a 
                            ...
                            onClick={() => {
                              if (onTrackEvent) {
                                // ⭐ Read the active message body string to accurately match the bank card
                                const correctId = extractCardFromContent(chat.content || '');
                                onTrackEvent('click_apply', correctId);
                              }
                            }}
            ...
            {chat.role === 'ai' && chat.applicationUrl && (
              <div className="mt-5">
                <a 
                  ...
                  onClick={() => {
                    if (onTrackEvent) {
                      // ⭐ Read the active message body string to eliminate stale state variables
                      const correctId = extractCardFromContent(chat.content || '');
                      onTrackEvent('click_apply', correctId);
                    }
                  }}
